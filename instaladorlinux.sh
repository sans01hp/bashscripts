...
printf "%b[+] Atualizando pacotes e instalando dependências...%b\n" "$CYAN" "$RESET"
sudo apt update -y && sudo apt install -y neovim git curl unzip python3-pip
...
-- ===== Alternar exibição de números =====
-- Pressione Ctrl+n para ligar/desligar números e ruler
...
# -------- PATH do Go e instalação de LSPs --------
export PATH="${HOME}/go/bin:${PATH}"
python3 -m pip install --user pynvim || true
python3 -m pip install --user "python-lsp-server[all]" || printf "%bFalha instalando pylsp%b\n" "$YELLOW" "$RESET"
...
# -------- Lazy sync plugins --------
nvim --headless "+Lazy sync" +qa || printf "%bLazy sync falhou — abra o nvim e rode :Lazy sync%b\n" "$YELLOW" "$RESET"

# -------- Finalização --------
printf "%b[✔] Instalação do Neovim concluída!%b\n" "$GREEN_BOLD" "$RESET"
printf "%bExecute:%b %snvim%s %bpara abrir o editor e testar o autocomplete.%b\n" "$YELLOW" "$RESET" "$CYAN" "$RESET" "$GREEN" "$RESET"
set +eo pipefail
